...
const contractAddress = "0xaA2E0cAd8252fb0adE7903C345bb0C10394F3C5d";
//comment
const dApp = {
  ethEnabled: function() {
  ...
  collectVars: async function() {
    ...
    for (let i = 1; i <= this.totalSupply; i++) {
      try {
        ...
        this.tokens.push({
          tokenId: i,
          lowestrate: Number(await this.marsContract.methods.lowestRate(i).call()),
          highestBid: 1000000000000000000,
          auctionEnded: Boolean(await this.marsContract.methods.auctionEnded1(i).call()),
  ...
  updateUI: async function() {
    ...
    this.tokens.forEach((token) => {
      try {
        let endAuction = `<a token-id="${token.tokenId}" class="dapp-admin" style="display:none;" href="#" onclick="dApp.endAuction(event)">End Loan Rate Auction</a>`;
        let bid = `<a token-id="${token.tokenId}" href="#" onclick="dApp.bid(event);">Loan Rate Bid</a>`;
            ...
            `<div class="col m6">
              <div class="card">
                ...
                <div class="card-action">
                  ...
                  <input type="number" min="${token. lowestrate - 1}" name="dapp-lowestrate" value= "${token.lowestrate -1}" ${token.auctionEnded ? 'disabled' : ''}>
  ...
  bid: async function(event) {
    ...
    const wei = 1000000000000000000  //Number($(event.target).prev().prev().val());
    const interest_rate = Number($(event.target).prev().val());
    console.log("Trying to bid!");
    console.log(wei);
    console.log(interest_rate);
    await this.marsContract.methods.bid(tokenId, interest_rate).send({from: this.accounts[0], value: wei}).on("receipt", async (receipt) => {
  ...
  endAuction: async function(event) {
    const tokenId = $(event.target).attr("token-id");
    await this.marsContract.methods.endAuction1(tokenId).send({from: this.accounts[0]}).on("receipt", async (receipt) => {
      M.toast({ html: "Transaction Mined! Refreshing UI..." });
      await this.updateUI();
    });
  },
  ...
  main: async function() {
    ...
    this.auctionJson = await (await fetch("./LoanAuction.json")).json();
